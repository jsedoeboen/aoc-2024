...
for(antenna, coords) in antennas.items():
    for i in range(len(coords)):
        for j in range(len(coords)):
            ...
            first_anti = (antenna_partner_coords[0] + dx, antenna_partner_coords[1] + dy)
            second_anti = (antenna_coords[0] - dx, antenna_coords[1] - dy)
            # print(f"antenna: {antenna} {antenna_coords} {antenna_partner_coords}")

            # check if in grid
            if 0 <= first_anti[0] < width and 0 <= first_anti[1] < height:
                anti_antennas.add(first_anti)

            if 0 <= second_anti[0] < width and 0 <= second_anti[1] < height:
